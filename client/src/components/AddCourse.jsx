...
//           }}
//         >
//           <div className="bg-dark p-4 rounded shadow w-100" style={{ maxWidth: '640px' }}>
//             <h4 className="fw-bold mb-4 text-warning">Add New Course</h4>
...
export default function AddCourse() {
  const { community } = useParams();  // read from /add-course/:community
  const navigate = useNavigate();
  ...
  // If for some reason the route was hit without a community param:
  if (!community) {
    toast.error("❌ Community name missing!");
    return null;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("✅ Course added successfully!");
      setTimeout(() => navigate(`/c/${encodeURIComponent(community)}`), 1500);
    } catch (err) {
      console.error(err.response?.data || err.message);
      const serverMsg =
        ...
        err.response?.data ||
        err.message;
      toast.error(`❌ Failed to add course: ${serverMsg}`);
    }
  };
