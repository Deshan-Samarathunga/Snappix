...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [userRes, postsRes, commentsRes, likesRes, dislikesRes, coursesRes] = await Promise.all([
          axios.get('http://localhost:8080/api/posts', { headers }),
          axios.get('http://localhost:8080/api/user/comments', { headers }),
          axios.get('http://localhost:8080/api/user/likes', { headers }),
          axios.get('http://localhost:8080/api/user/dislikes', { headers }),
          axios.get('http://localhost:8080/api/user/courses', { headers }) // New endpoint for courses
        ]);
    ...
    switch (tab) {
      ...
      case 'courses':
        return courses.length ? (
          <div>
            <h5 className="text-info mb-3"><FontAwesomeIcon icon={faBookOpen} className="me-2" />My Courses</h5>
            <ul className="list-group">
              {courses.map((course, i) => (
                <li key={i} className="list-group-item bg-dark text-light mb-2">
                  <div className="d-flex justify-content-between align-items-center">
                    <div>
                      <strong>{course.title}</strong>
                      <div style={{ fontSize: '0.9em' }}>{course.description}</div>
                    </div>
                    <Link to={`/course/${course.id}`} className="btn btn-info btn-sm">
                      View
                    </Link>
                  </div>
                </li>
  ...
  const tabs = [
    ...
    { key: 'courses', label: 'Courses' }, // New tab
  ];
