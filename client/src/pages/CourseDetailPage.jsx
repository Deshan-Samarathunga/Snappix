  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8080/api/courses/${id}`, {
      ...
      .then(res => {
        ...
        if (res.data.communityName) {
          axios.get(`http://localhost:8080/api/communities/name/${res.data.communityName}`, {
            headers: { Authorization: `Bearer ${token}` }
          })
            .then(commRes => {
              ...
              const members = (commRes.data.members || []).map(e => e?.toLowerCase().trim());
              const contributors = (commRes.data.contributors || []).map(e => e?.toLowerCase().trim());
              const createdBy = commRes.data.createdBy?.toLowerCase().trim();
              ...
              const isModerator = createdBy === userEmail;

              // Debugging
              console.log({ isMember, isContributor, isModerator });

              setShowEnrollButton(Boolean(isMember && !isContributor && !isModerator));
            })
            .catch(() => {
