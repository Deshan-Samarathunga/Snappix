...
public class CommunityController {
    ...
    public ResponseEntity<?> createCommunity(
        ...
        try {
            ...
            // Add creator to moderators list if you have one
            // community.setModerators(new ArrayList<>(List.of(email)));
    ...
    // Promote a member to moderator (if you use moderators)
    ...
    public ResponseEntity<?> promoteMember(
            ...
            @RequestHeader("Authorization") String authHeader) {
        String token = authHeader.replace("Bearer ", "");
        String requester = jwtUtil.extractEmail(token);
        String memberToPromote = body.get("email");
        ...
        if (!community.getCreatedBy().equalsIgnoreCase(requester)) {
            return ResponseEntity.status(403).body("Only the community creator can promote members");
        }
        if (community.getCreatedBy().equalsIgnoreCase(memberToPromote)) {
            return ResponseEntity.badRequest().body("Creator is already a moderator");
        }
        if (community.getMembers() == null || !community.getMembers().contains(memberToPromote)) {
            return ResponseEntity.badRequest().body("User is not a member of this community");
        }
        if (community.getModerators() != null && community.getModerators().contains(memberToPromote)) {
            return ResponseEntity.badRequest().body("User is already a moderator");
        }

        community.getModerators().add(memberToPromote);
        communityRepo.save(community);
        return ResponseEntity.ok("Member promoted to moderator");
    }
}
